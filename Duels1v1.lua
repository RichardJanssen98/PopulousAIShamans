...
AIShamanCyan = AIShaman:new(nil, TRIBE_CYAN, 1, 1, 1, 0, 1, 0, 0, 1, 1, 10000, 7)
...
function HandlePraying(pn, prayTick, headMarker, enemyHeadX, enemyHeadZ)
  ...
  if (shaman ~= nil) then
    ...
    SearchMapCells(CIRCULAR, 0, 0, 2, world_coord2d_to_map_idx(c2d), function(me)
      ...
      elseif (everyPow(prayTick, 1) and cyanIsGoingToPray == 0 and cyanGoingToDefend == 0) then
  ...
  if (cyanIsGoingToPray == 1 and (enemyShaman ~= nil or shaman == nil)) then
    cyanIsGoingToPray = 0
    cyanPrayTick = 0
  end
end